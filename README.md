...
The memory safety check above only runs *at switch time* — it can't help when a model sits loaded for a long session and free RAM keeps falling anyway (rapid-mlx's own prefix cache growing, KV-cache growth on a long conversation, Metal buffer fragmentation that never gets reclaimed) until the machine locks up or macOS force-restarts it under memory pressure, with nothing having "switched" to trigger the existing check. `memwatchdog.go` runs a background goroutine for the lifetime of the process, independent of any load/switch request: every `memory_watchdog_interval_seconds` (default 30) it checks free+inactive RAM as a percentage of total physical memory, and if that drops below `memory_watchdog_threshold_percent` (default 15, both configurable in `models.json`), it restarts whatever's currently loaded on each of the three gateway-spawned pools (chat, OCR, FLUX) — one at a time, through the same cross-process-locked `loadModel` entry point `unified-gateway load` uses. Ollama is exempt, same reasoning as the switch-time check: it's never spawned/killed by the gateway.
...
- **`🟢 RAM: X/Y GB free (Z%)`** — live free-vs-total physical memory (`vm_stat` free+inactive pages vs `sysctl hw.memsize`), refreshed on the same tick as everything else. The dot is cosmetic (🟢/🟡/🔴 by free-percent thresholds) and separate from the gateway's own memory watchdog, which independently force-restarts every loaded backend if free RAM drops below a configurable threshold (default 15%, checked every 30s) — this line is just the human-readable view of the same free-RAM number that watchdog acts on.
