...
| `models.<name>.tool_call_parser` | Overrides rapid-mlx's own tool-call-format auto-detection (which matches regexes against the model's **path string**, not `model_type`). A rebranded/repackaged checkpoint whose directory name doesn't contain a recognizable family name (e.g. a Qwen3-based model not named anything with "qwen3" in it) silently gets no tool-call parser at all from rapid-mlx's side — tool calls just don't work, with no error. Set this to the correct parser name (e.g. `"hermes"`) for any such model; passed as `--tool-call-parser <value> --enable-auto-tool-choice` in `launchMLX`. Leave unset for models whose path already self-identifies correctly |
| `models.<name>.ds4_ssd_streaming` | `"ds4"`-backend only: passes `--ssd-streaming` to `ds4-server` instead of its default full-residency load. ds4 has no memory headroom check anywhere in full-residency mode — it mmaps the whole model and only finds out there wasn't enough room left for prefill's transient Metal buffers when that prefill call fails with a raw `kIOGPUCommandBufferCallbackErrorOutOfMemory`. `--ssd-streaming` is ds4's own opt-in path with real headroom accounting (reserves an explicit prefill-headroom budget up front). Verified directly on `antirez`/`ds4flash.gguf` (90.88 GiB) on a 128 GiB M5 Max: full residency failed prefill with that exact OOM error; the same model with this flag set planned 50.20 GiB total and completed a real chat completion successfully. Only worth setting for models large enough that full residency doesn't leave comfortable headroom — smaller GGUFs can leave this unset |
...
**Verified end-to-end with `flux2-klein-4b`, `flux1-dev`, and `z-image-turbo`** (real images generated through the full stack: gateway load → `flux-server/server.py` → unicorn-server's `/v1/media/generate`). One caveat found the hard way: not every HuggingFace repo claiming to be a quantized FLUX checkpoint works with mflux — repos packaged for [flux.swift](https://github.com/mzbac/flux.swift) (e.g. `mzbac/flux1.dev.4bit.mlx`) use a different quantization packing and fail inside mflux's text encoder (`[dequantize] The matrix should be given as a uint32`) despite loading without error. Look for checkpoints explicitly quantized *with* mflux itself (e.g. `dhairyashil/FLUX.1-dev-mflux-4bit`) instead.

**`z-image-turbo`** ([Tongyi-MAI/Z-Image-Turbo](https://huggingface.co/Tongyi-MAI/Z-Image-Turbo) via [filipstrand/Z-Image-Turbo-mflux-4bit](https://huggingface.co/filipstrand/Z-Image-Turbo-mflux-4bit)) was added after FLUX.2-klein-4B's prompt adherence and text rendering turned out to be noticeably weaker than expected — Klein is FLUX.2's speed-optimized distilled variant, not its flagship. Z-Image-Turbo (6B params, 9 steps) benchmarks *better* than full FLUX.2 Dev (32B) on text-rendering word error rate despite being far smaller, and that held up in direct testing here too. Hit one real bug getting it working: `ZImage.generate_image` crashes with `RuntimeError: There is no Stream(gpu, 1) in current thread` when called from any thread other than the one that loaded the model — reproduced independent of the HTTP layer, it's an MLX GPU-stream thread-locality issue inside mflux's Z-Image implementation. `flux-server/server.py` used to run `ThreadingHTTPServer` (one thread per request); switched to a plain single-threaded `HTTPServer`, since there's no real concurrency to lose — one client, one active generation at a time, GPU compute is serialized regardless. `flux1-dev`/`flux2-klein-4b` never happened to trip this, purely by luck of not sharing Z-Image's internal stream-handling path.

## Memory safety check

Before a `loadModel` call for a locally-spawned backend (`mlx` or `ds4`) kills the outgoing process and launches the new one, it checks whether there's actually enough RAM: the new model's on-disk weight size (summed `.safetensors` shards for `mlx`, the raw `.gguf` size for `ds4`) plus a 10% margin is compared against currently-available RAM plus what killing the current backend will free up (its live RSS). If that doesn't add up, the switch is refused with a clear error and the current backend is left running untouched — rather than launching into a machine that's about to start swapping heavily or get killed outright by macOS under memory pressure (this is what caused a full forced reboot in practice: a large model plus an already-loaded one together left no headroom at all). Ollama is exempt — it manages its own memory and isn't spawned/killed by the gateway.

**Available RAM is computed as total physical memory minus the sum of every live process's RSS** (`ps -axo rss=`), not `vm_stat`'s "Pages free + Pages inactive" (an earlier version, and still a common approach elsewhere). That vm_stat-based figure measurably undercounts what's actually available on macOS: it ignores active/speculative/file-backed cache pages that aren't part of any live process's resident set but get reclaimed first under real memory pressure — disk cache and read-ahead, not real application memory. On a 128GB test machine this under-count was large enough to block loading models that had real headroom (~64GB reported free vs. ~93GB actually available for the same system state). The total-minus-RSS figure was confirmed to match `mactop`'s own reading directly.
...
**TurboQuant and PFlash, for large-context workloads.** `launchMLX` passes `--kv-cache-turboquant k8v4` (K=8-bit Walsh-Hadamard, V=4-bit Lloyd-Max) to every mlx model *except* 8-bit weight-quantized ones (`ModelConfig.QuantBits == 8`), which get the bare `v4` mode instead. k8v4 measured a genuine +30% decode win at long context on a 4-bit model (qw3635: 48.8 → 63.6 tok/s at ~80k tokens) with no regression at short context — but on an 8-bit weight-quantized model (qw3coder8bit) the same flag caused a 13x *regression* (79 → 5.9 tok/s), reproduced twice, most likely an interaction between the KV-cache's own 8-bit quantization and the model weights' 8-bit quantization on the same Metal kernel path. Only ever benchmarked against 4-bit/5-bit weights before making k8v4 the default, hence the gate now being strict on `QuantBits` rather than model size or type. `--pflash auto` (long-prompt prefill compression) is only added when `!m.HasVision`: rapid-mlx hard-refuses `--pflash` for any multimodal model, even one forced into `--text-only`/`--no-mllm` here, since its own check looks at the base model's vision capability rather than the runtime override. Found this the hard way — with both flags on unconditionally, rapid-mlx exited immediately with `--pflash is not supported for multimodal models`, and since the process died before ever binding the port, it looked from the gateway's side exactly like a hung load rather than a fast, explained failure. Every current `qwen3_5`-type catalog entry has vision, so PFlash presently only actually applies to the non-vision mlx models (DeepSeek, GPT-OSS, Mixtral).

## Background watchdogs

Two independent goroutines poll every `MemoryWatchdogIntervalSeconds` (default 30s) for as long as the gateway process is up, entirely separate from the per-switch memory safety check above (which only runs at `loadModel` time):

- **Memory watchdog** — if available RAM (total physical RAM minus the sum of every live process's RSS — see [Memory safety check](#memory-safety-check) above for why, not `vm_stat` free+inactive) drops below `MemoryWatchdogThresholdPercent` of total physical RAM (default 15%, `models.json`, negative disables it), every currently-loaded gateway-managed backend (chat, OCR, FLUX pools) is force-restarted with the same model, on a 5-minute cooldown between triggers. There's no cache-trim endpoint on any of these backends (rapid-mlx/ds4-server/flux-server.py), so kill-and-relaunch is the only form of "compaction" actually available — it forces macOS to reclaim the process's full resident memory, including whatever accumulated prefix-cache/fragmentation caused the pressure. Ollama is exempt, same as the per-switch check.
- **Stall watchdog** — if a chat request sits with `rapid_mlx_requests_running > 0` while rapid-mlx's own `rapid_mlx_steps_executed_total` scheduler-step counter stops advancing for `StallWatchdogThresholdSeconds` (default 300s, negative disables it), the chat backend is force-restarted. Deliberately watches the *scheduler step* counter rather than completion tokens: a legitimate long prefill (a huge prompt, or a client-side context-compaction call summarizing a large chunk of history) produces zero completion tokens by definition until decode starts, which looks identical to a genuine hang if you're only watching output tokens — and killing a real-but-slow compaction mid-flight just means the client retries the same call and never gets far enough to finish it, an infinite kill/retry loop instead of a one-time recovery. `steps_executed_total` increments on every scheduler step, prefill chunks included, so it's live exactly when the engine is doing anything at all. Only the chat pool is covered — OCR/FLUX run different servers that don't expose this counter.

Both watchdogs fire a native macOS notification (`osascript display notification`) when they act, and print to the gateway's own log (`~/Library/Logs/unified-gateway/stdout.log`). A restart triggered by either shares the same in-flight-load flag the gateway's request-time auto-reload path uses, so the two can't pile up into two back-to-back full reloads for one incident.

## Memory watchdog

The memory safety check above only runs *at switch time* — it can't help when a model sits loaded for a long session and available RAM keeps falling anyway (rapid-mlx's own prefix cache growing, KV-cache growth on a long conversation, Metal buffer fragmentation that never gets reclaimed) until the machine locks up or macOS force-restarts it under memory pressure, with nothing having "switched" to trigger the existing check. `memwatchdog.go` runs a background goroutine for the lifetime of the process, independent of any load/switch request: every `memory_watchdog_interval_seconds` (default 30) it checks available RAM (total minus the sum of every live process's RSS) as a percentage of total physical memory, and if that drops below `memory_watchdog_threshold_percent` (default 15, both configurable in `models.json`), it restarts whatever's currently loaded on each of the three gateway-spawned pools (chat, OCR, FLUX) — one at a time, through the same cross-process-locked `loadModel` entry point `unified-gateway load` uses. Ollama is exempt, same reasoning as the switch-time check: it's never spawned/killed by the gateway.

There's no cache-trim endpoint exposed by rapid-mlx/ds4-server/flux-server.py to call instead — a restart (kill + fresh launch of the same model) is the only form of compaction actually available from outside these opaque processes, and it's a real one: it forces macOS to reclaim 100% of that process's resident memory, including whatever accumulated cache or fragmentation caused the pressure. This does drop whatever request that backend was mid-handling at the restart moment — an accepted tradeoff, since the alternative observed in practice (full lockup or forced reboot) loses far more. A 5-minute cooldown between watchdog-triggered restarts prevents thrashing if pressure doesn't clear immediately. Setting `memory_watchdog_threshold_percent` to a negative number disables the watchdog entirely (free% can never go negative, so the check never fires).
...
- **`🟢 RAM: X/Y GB free (Z%)`** — live available-vs-total physical memory (total via `sysctl hw.memsize` minus the sum of every live process's RSS via `ps -axo rss=` — not `vm_stat` free+inactive, which undercounts real availability, see [Memory safety check](#memory-safety-check)), refreshed on the same tick as everything else. The dot is cosmetic (🟢/🟡/🔴 by free-percent thresholds) and separate from the gateway's own memory watchdog, which independently force-restarts every loaded backend if available RAM drops below a configurable threshold (default 15%, checked every 30s) — this line is just the human-readable view of the same number that watchdog acts on.
- **`rapid-mlx`** / **`ds4`** — one entry per local backend, each a submenu with its own **Start**, **Stop**, and a list of the models configured for that backend (`models.json`'s `"backend": "mlx"` / `"ds4"` entries); clicking a model loads it directly. The 🟢/🔴 dot and model name are read straight from the real process's own command line (which one owns the backend port, and its `--served-model-name`/`--model` argument) — not from a state file this tool wrote earlier, so it's still correct even if that process was started or replaced by something else.
  - Each model entry is labelled `<name> (<quant info>) (<short-id>, <context>, <disk size>)` — e.g. `Qwen3-Coder-30B-A3B (MoE, official, 6-bit) (qw3coder6bit, 256K, 23GB)` — and prefixed with a 🟢/🟡/🔴 dot that's a rough, hand-maintained call on real-world coding quality (`cmd/menubar/codingquality.go`), not a benchmark score. The list itself is sorted by on-disk size, smallest first, rather than alphabetically, since that's the number that actually decides whether a model coexists with whatever else is running; entries with no computable disk size (e.g. Ollama-backed) sort last. Disk size is read live off the model's `.safetensors` shards, so a model that's still mid-download shows its current partial size until it finishes.
