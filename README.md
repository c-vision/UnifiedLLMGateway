...
**Media models run concurrently with the active chat model, never instead of it.** Switching between two *chat* models is deliberately exclusive — they share `Config.BackendPort`, and loading one kills whatever was there, because rapid-mlx only serves one model per process. Media models don't follow that rule: they get their own `Config.MediaBackendPort` (`backend_port + 1` unless set explicitly) and their own independent process lifecycle, so requesting OCR while a chat model is mid-conversation loads OCR alongside it rather than killing the chat backend to make room — the same non-exclusive relationship Ollama already has with the mlx/ds4 backend, just for a locally-spawned process instead of an external daemon. Both adapters route a request to whichever port actually matches its `model` field's `kind`, and the memory-safety check above only ever credits/kills whatever was previously on *that* model's own port, never the other one's — so two rapid-mlx processes genuinely resident at once is expected, not a bug, and sized for accordingly.
...
- **`Media Models (OCR, etc.)`** — only appears if `models.json` has any `"kind": "media"` entries. Own **Start**/**Stop** and its own model list, exactly like `rapid-mlx`/`ds4` above — but on `Config.MediaBackendPort`, a separate port from the chat backend. Starting or stopping something here never touches whatever chat model is currently loaded, and vice versa: the two run concurrently, not exclusively, the same relationship `Ollama` already has with the chat backend.
