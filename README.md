...
- **`Media Models (OCR, etc.)`** — only appears if `models.json` has any `"kind": "media"` entries. Same click-to-load behavior as the backend sections above, just kept visually separate from the everyday chat-model lists regardless of which backend actually serves it.
